...
public class DES {
    ...
    static int[] IP = {58, 50, 42, 34, 26, 18, 10, 2, 60, 52, 44, 36, 28, 20, 12, 4, 62, 54, 46, 38, 30, 22, 14, 6,
            64,
    ...
    static int[] PC2 = {14, 17, 11, 24, 1, 5, 3, 28, 15, 6, 21, 10, 23, 19, 12, 4, 26, 8, 16, 7, 27, 20, 13, 2, 41,
            52,
            31, 37, 47, 55, 30, 40, 51, 45, 33, 48, 44, 49, 39, 56, 34, 53, 46, 42, 50, 36, 29, 32};

    static int[] E = {32, 1, 2, 3, 4, 5, 4, 5, 6, 7, 8, 9, 8, 9, 10, 11, 12, 13, 12, 13, 14, 15, 16, 17, 16, 17,
            18,
            19, 20, 21, 20, 21, 22, 23, 24, 25, 24, 25, 26, 27, 28, 29, 28, 29, 30, 31, 32, 1};

    static int[] P = {16, 7, 20, 21, 29, 12, 28, 17, 1, 15, 23, 26, 5, 18, 31, 10, 2, 8, 24, 14, 32, 27, 3, 9, 19,
            13,
            30, 6, 22, 11, 4, 25};

    static int[] IP_1 = {40, 8, 48, 16, 56, 24, 64, 32, 39, 7, 47, 15, 55, 23, 63, 31, 38, 6, 46, 14, 54, 22, 62,
            30,
    ...
    public static long permuteKey(long input, int size, int[] map) {
        long permutedKey = 0;
        for (int i = 0; i < map.length; i++) {
            int originalPosition = size - map[i];
            long bit = (input >> originalPosition) & 1;
            permutedKey |= (bit << (map.length - i - 1));
        }
        return permutedKey;
    }

    private static long initialPermutation(long plaintext) {
        return permuteKey(plaintext, 64, IP);
    }

    private static long finalPermutation(long text) {
        return permuteKey(text, 64, IP_1);
    }
    ...
    private static Pair<Long, Long> keyExpansion(long key, int shift) {
        ...
        h1 = ((h1 << shift) | (h1 >>> (28 - shift))) & 0xfffffffL;
        h2 = ((h2 << shift) | (h2 >>> (28 - shift))) & 0xfffffffL;

        long nextKey = (h1 << 28) | h2;
        long subKey = permuteKey(nextKey, 56, PC2);
